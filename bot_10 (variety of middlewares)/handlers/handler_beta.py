...
from filters.filter_quartet import FiftyPercentFilter
...
router.message.middleware(Middleware(middleware_name="Middleware_D(beta)"))
router.message.outer_middleware(Middleware(middleware_name="Middleware_B(beta)"))
...
@router.message(Command("start"), FiftyPercentFilter(), flags={"handler_name": "Handler_beta"})
